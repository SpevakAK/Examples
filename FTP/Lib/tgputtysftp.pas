...
const
  MinimumLibraryBuildNum = 8;
  cDummyClearedErrorCode = -1000;
  // this error code means there was no real error code

  cConfCount = 85; // 85 since v0.76, 84 since PuTTY version 0.74, previously 83
  cDumpSettingsFile = false;

type
  TGPuttySFTPException = class(Exception);

  TOnMessage = procedure(const Msg: AnsiString; const isstderr: Boolean)
    of object;
  TOnCheckpoint = procedure(const Msg: AnsiString; const kind: Byte) of object;
  TOnProgress = function(const bytescopied: Int64; const isupload: Boolean)
    : Boolean of object;
  ...
  private
    ...
    FUploadStream, FDownloadStream: TStream;
    FConnected: Boolean;
  ...
  public
    ...
    procedure SetModifiedDate(const AFileName: AnsiString;
      const ATimestamp: TDateTime; const isUTC: Boolean);
    ...
    procedure MoveEx(const AFromName, AToName: AnsiString;
      const MoveFlags: Integer);
    procedure DeleteFile(const AName: AnsiString);

    procedure UploadFile(const ALocalFilename, ARemoteFilename: AnsiString;
      const anAppend: Boolean);
    procedure DownloadFile(const ARemoteFilename, ALocalFilename: AnsiString;
      const anAppend: Boolean);

    procedure UploadStream(const ARemoteFilename: AnsiString;
      const AStream: TStream; const anAppend: Boolean);
    procedure DownloadStream(const ARemoteFilename: AnsiString;
      const AStream: TStream; const anAppend: Boolean);
    procedure DownloadStreamP(const ARemoteFilename: AnsiString;
      const AStream: TStream; const anAppend: Boolean);

    function OpenFile(const apathname: AnsiString; const anopenflags: Integer;
      const Attrs: Pfxp_attrs): TSFTPFileHandle;
    function CloseFile(var fh: TSFTPFileHandle): Integer;

    function xfer_upload_init(const fh: TSFTPFileHandle; const offset: UInt64)
      : TSFTPTransfer;
    ...
    function xfer_download_init(const fh: TSFTPFileHandle; const offset: UInt64)
      : TSFTPTransfer;
    function xfer_download_preparequeue(const xfer: TSFTPTransfer): Boolean;
    function xfer_download_data(const xfer: TSFTPTransfer; var buffer: PByte;
    ...
    procedure SetBooleanConfigValue(const OptionName: AnsiString;
      const OptionValue: Boolean);
    procedure SetIntegerConfigValue(const OptionName: AnsiString;
      const OptionValue: Integer);
    ...
    property ConnectionTimeoutTicks: Integer read GetConnectionTimeoutTicks
      write SetConnectionTimeoutTicks;
    ...
    property ProxyUserName: AnsiString read GetProxyUserName
      write SetProxyUserName;
    property ProxyPassword: AnsiString read GetProxyPassword
      write SetProxyPassword;

    property ProgressIntervalMS: Integer read FProgressIntervalMS
      write FProgressIntervalMS;
    property ProgressAfterBytes: Int64 read FProgressAfterBytes
      write FProgressAfterBytes;
    ...
    property OnVerifyHostKey: TOnVerifyHostKey read FOnVerifyHostKey
      write FOnVerifyHostKey;
  end;

implementation

var
  GPuttyConfigIndex: tStringList;
  GPuttyConfigCS: TCriticalSection;

function ls_callback(const names: Pfxp_names; const libctx: PTGLibraryContext)
  : Boolean; cdecl;
var
...
function getpassword_callback(const prompt: PAnsiChar; const echo: Boolean;
  const cancel: System.PBoolean; const libctx: PTGLibraryContext)
  : PAnsiChar; cdecl;
var
  TGPSFTP: TTGPuttySFTP;
begin
  ...
  if TGPSFTP.FPasswordAttempts > 3 then
  begin
    cancel^ := true;
    if Assigned(TGPSFTP.OnMessage) then
      TGPSFTP.OnMessage
        (AnsiString('Password was rejected, or no password given for ') + prompt
        + AnsiString('.') + sLineBreak, true);
  end
  else
  begin
...
procedure printmessage_callback(const Msg: PAnsiChar; const kind: Byte;
...
function progress_callback(const bytescopied: Int64; const isupload: Boolean;
...
function get_input_callback(linebuf: PAnsiChar; const maxchars: Integer;
  const libctx: PTGLibraryContext): Boolean; cdecl;
var
  line: AnsiString;
  TGPSFTP: TTGPuttySFTP;
...
begin
  ...
  if Assigned(TGPSFTP.OnGetInput) then
  begin
...
function read_from_stream(const offset: UInt64; const buffer: Pointer;
  const bufsize: Integer; const libctx: PTGLibraryContext): Integer; cdecl;
var
  TGPSFTP: TTGPuttySFTP;
begin
  TGPSFTP := TTGPuttySFTP(libctx.Tag);
  if Assigned(TGPSFTP.FUploadStream) then
  begin
...
function write_to_stream(const offset: UInt64; const buffer: Pointer;
  const bufsize: Integer; const libctx: PTGLibraryContext): Integer; cdecl;
var
  TGPSFTP: TTGPuttySFTP;
begin
  TGPSFTP := TTGPuttySFTP(libctx.Tag);
  if Assigned(TGPSFTP.FDownloadStream) then
  begin
...
procedure raise_exception_callback(const Msg: PAnsiChar;
  const srcfile: PAnsiChar; const line: Integer;
  const libctx: PTGLibraryContext); cdecl;
begin
{$IFDEF SFFS}
  if IndyLogging then
  begin
    WriteLn(IndyLog, 'NOW RAISING TTGPuttySFTP exception ' + AnsiString(Msg) +
      ' at line ' + AnsiString(IntToStr(line)) + ' in ' + AnsiString(srcfile));
    CloseFile(IndyLog);
    Append(IndyLog);
    if Logging then
    begin
      WriteLn(LogFile, 'NOW RAISING TTGPuttySFTP exception ' + AnsiString(Msg) +
        ' at line ' + AnsiString(IntToStr(line)) + ' in ' +
        AnsiString(srcfile));
      CloseFile(LogFile);
      Append(LogFile);
    end;
...
{$IFDEF UNICODE}Utf8ToString{$ENDIF}(AnsiString(Msg)) + ' at line ' + IntToStr(line) + ' in ' +
{$IFDEF UNICODE}Utf8ToString{$ENDIF}(AnsiString(srcfile)));
end;

function verify_host_key_callback(const host: PAnsiChar; const port: Integer;
  const keytype: PAnsiChar; const keystr: PAnsiChar;
  const fingerprint: PAnsiChar; const verificationstatus: Integer;
  const storehostkey: System.PBoolean; const libctx: PTGLibraryContext)
  : Boolean; cdecl;
var
  TGPSFTP: TTGPuttySFTP;
begin
  ...
  if Assigned(TGPSFTP.OnVerifyHostKey) then
    Result := TGPSFTP.OnVerifyHostKey(host, port, fingerprint,
...
begin
  ...
  FLastMessages := '';
  Fcontext.fxp_errtype := cDummyClearedErrorCode; // "clear" error field
  res := tgsftp_connect(PAnsiChar(FHostName), PAnsiChar(FUserName), FPort,
    PAnsiChar(FPassword), @Fcontext);
...
begin
  ...
  if tgputtylibbuild < MinimumLibraryBuildNum then
    raise Exception.Create('tgputtylib is too old, its build number is ' +
      IntToStr(tgputtylibbuild) + ', but we need a minimum of ' +
      IntToStr(MinimumLibraryBuildNum));
  ...
  if tgputty_initcontext(ord(verbose), @Fcontext) <> 0 then
    raise TGPuttySFTPException.Create
      ('tgputty_initcontext failed - incorrect tgputtylib version?');

  if not tgputty_getconfigarrays(@FConfTypes, @FConfSubTypes, @FConfNames,
    @FConfCount) then
    raise TGPuttySFTPException.Create
      ('tgputty_getconfigarrays failed - incorrect tgputtylib version?');

  if FConfCount <> cConfCount then
    printmessage_callback
      (PAnsiChar(AnsiString('Possibly tgputtylib version mismatch, it has ') +
...
begin
  ...
  try
    if not Assigned(GPuttyConfigIndex) then
    begin
      GPuttyConfigIndex := tStringList.Create;
      GPuttyConfigIndex.Sorted := true;
      for i := 0 to FConfCount - 1 do
        GPuttyConfigIndex.AddObject(string(FConfNames[i]),
          TObject(NativeUInt(i)));
    end;
  finally
...
procedure TTGPuttySFTP.DownloadFile(const ARemoteFilename, ALocalFilename
  : AnsiString; const anAppend: Boolean);
var
  res: Integer;
begin
  FLastMessages := '';
  Fcontext.fxp_errtype := cDummyClearedErrorCode; // "clear" error field
  res := tgsftp_getfile(PAnsiChar(ARemoteFilename), PAnsiChar(ALocalFilename),
    anAppend, @Fcontext);
  if res <> 1 then
    raise TGPuttySFTPException.Create(MakePSFTPErrorMsg('tgsftp_getfile'));
end;

procedure TTGPuttySFTP.DownloadStream(const ARemoteFilename: AnsiString;
  const AStream: TStream; const anAppend: Boolean);
var
  res: Integer;
begin
...
procedure TTGPuttySFTP.DownloadStreamP(const ARemoteFilename: AnsiString;
  const AStream: TStream; const anAppend: Boolean);
var
  fh: TSFTPFileHandle;
  xfer: Pointer;
  ...
  canceled: Boolean;
  Attrs: fxp_attrs;
  starttick, idlesincetick, TotalBytes, LastProgressBytes, lastprogresstick,
    endtick: UInt64;
  buf, vbuf: PByte;
...
{$ENDIF}
  ...
  fh := tgputty_openfile(PAnsiChar(ARemoteFilename), SSH_FXF_READ, nil,
    @Fcontext);
  ...
  while (Result and not xfer_done(xfer) and not canceled and
    not Fcontext.Aborted) do
  begin
    CP('psgetf62');
    ...
    while Result and xfer_download_data(xfer, vbuf, len) do
    begin
      CP('psgetf71');
      buf := vbuf;
      ...
      begin
        ...
        if Assigned(OnProgress) and
          ...
          (GetTickCount() - lastprogresstick >= FProgressIntervalMS)) then
        begin
          CP('psgetf81');
          if not OnProgress(TotalBytes, false) then
          begin
            CP('psgetf82');
            canceled := true;
            Result := false;
    ...
    begin
      if idlesincetick = 0 then
        idlesincetick := GetTickCount()
      else if GetTickCount() - idlesincetick > Fcontext.TimeoutTicks then
      begin
        CP('psgetf95');
        printmessage_callback('Timeout error, no more data received.' +
          sLineBreak, 0, @Fcontext);
        Result := false;
        xfer_set_error(xfer);
        break;
      end;
  ...
  endtick := GetTickCount(); // TG
  if endtick - starttick = 0 then // TG
    endtick := starttick + 1; // prevent divide by zero ;=)

  CP('psgetf97');
  if Assigned(OnMessage) then
    OnMessage(AnsiString('Downloaded ' + IntToStr(TotalBytes) + ' Bytes in ' +
      IntToStr(endtick - starttick) + ' milliseconds, rate = ' +
      FloatToStr((TotalBytes / 1024) / (endtick - starttick)) + ' MB/sec.' +
      sLineBreak), false);
...
begin
  ...
  for i := 0 to FConfCount - 1 do
  begin
    Write(T, '      cPuttyConf_', FConfNames[i], '=''', FConfNames[i],
      '''; // ', txtPuttyConfTypes[FConfTypes[i]]);
    try
      if FConfSubTypes[i] > 0 then
        WriteLn(T, ' [', txtPuttyConfTypes[FConfTypes[i]], ']')
        // default ',tgputty_conf_get_int_int(i,0))
      else
        try
          case FConfTypes[i] of
            TYPE_BOOL:
              WriteLn(T, ' default ', tgputty_conf_get_bool(i, @Fcontext));
            TYPE_INT:
              WriteLn(T, ' default ', tgputty_conf_get_int(i, @Fcontext));
            TYPE_STR:
              WriteLn(T, ' default ''', tgputty_conf_get_str(i,
                @Fcontext), '''');
          else
            WriteLn(T);
          end;
...
begin
  ...
  Result := AnsiString('tgputtylib build ') +
    AnsiString(IntToStr(tgputtylibbuild)) +
    AnsiString(' based on PuTTY Release ') + strpv;
end;

function TTGPuttySFTP.GetProxyHost: AnsiString;
begin
  Result := tgputty_conf_get_str(GetPuttyConfIndex(cPuttyConf_proxy_host),
    @Fcontext);
end;

function TTGPuttySFTP.GetProxyPassword: AnsiString;
begin
  Result := tgputty_conf_get_str(GetPuttyConfIndex(cPuttyConf_proxy_password),
    @Fcontext);
end;

function TTGPuttySFTP.GetProxyPort: Integer;
begin
  Result := tgputty_conf_get_int(GetPuttyConfIndex(cPuttyConf_proxy_port),
    @Fcontext);
end;
...
begin
  Result := TProxyTypes
    (tgputty_conf_get_int(GetPuttyConfIndex(cPuttyConf_proxy_type), @Fcontext));
end;

function TTGPuttySFTP.GetProxyUserName: AnsiString;
begin
  Result := tgputty_conf_get_str(GetPuttyConfIndex(cPuttyConf_proxy_username),
    @Fcontext);
end;
...
procedure TTGPuttySFTP.GetStat(const AFileName: AnsiString;
  out Attrs: fxp_attrs);
begin
  FLastMessages := '';
  Fcontext.fxp_errtype := cDummyClearedErrorCode; // "clear" error field
...
begin
  if Fcontext.fxp_errtype >= 0 then
    Result := where + ': Error ' + IntToStr(Fcontext.fxp_errtype) + ', ' +
    {$IFDEF UNICODE}Utf8ToString{$ENDIF}(Fcontext.fxp_error_message)
  else
    Result := where + ': Unknown Error.' + sLineBreak +
    {$IFDEF UNICODE}Utf8ToString{$ENDIF}(LastMessages);
end;
...
procedure TTGPuttySFTP.MoveEx(const AFromName, AToName: AnsiString;
  const MoveFlags: Integer);
var
  res: Integer;
begin
  FLastMessages := '';
  Fcontext.fxp_errtype := cDummyClearedErrorCode; // "clear" error field
  res := tgsftp_mvex(PAnsiChar(AFromName), PAnsiChar(AToName), MoveFlags,
    @Fcontext);
  if res <> 1 then // 1 = success
    raise TGPuttySFTPException.Create(MakePSFTPErrorMsg('tgsftp_mvex'));
end;

function TTGPuttySFTP.OpenFile(const apathname: AnsiString;
  const anopenflags: Integer; const Attrs: Pfxp_attrs): TSFTPFileHandle;
begin
  Result := tgputty_openfile(PAnsiChar(apathname), anopenflags, Attrs,
    @Fcontext);
end;
...
procedure TTGPuttySFTP.SetFileSize(const AFileName: AnsiString;
  const ASize: Int64);
var
  Attrs: fxp_attrs;
begin
...
procedure TTGPuttySFTP.SetIntegerConfigValue(const OptionName: AnsiString;
  const OptionValue: Integer);
begin
  tgputty_conf_set_int(GetPuttyConfIndex(string(OptionName)), OptionValue,
    @Fcontext);
end;
...
procedure TTGPuttySFTP.SetModifiedDate(const AFileName: AnsiString;
  const ATimestamp: TDateTime; const isUTC: Boolean);
var
  Attrs: fxp_attrs;
  unixtime: Int64;
...
begin
  tgputty_conf_set_str(GetPuttyConfIndex(cPuttyConf_proxy_host),
    PAnsiChar(Value), @Fcontext);
end;

procedure TTGPuttySFTP.SetProxyPassword(const Value: AnsiString);
begin
  tgputty_conf_set_str(GetPuttyConfIndex(cPuttyConf_proxy_password),
    PAnsiChar(Value), @Fcontext);
end;

procedure TTGPuttySFTP.SetProxyPort(const Value: Integer);
begin
  tgputty_conf_set_int(GetPuttyConfIndex(cPuttyConf_proxy_port), ord(Value),
    @Fcontext);
end;

procedure TTGPuttySFTP.SetProxyType(const Value: TProxyTypes);
begin
  tgputty_conf_set_int(GetPuttyConfIndex(cPuttyConf_proxy_type), ord(Value),
    @Fcontext);
end;

procedure TTGPuttySFTP.SetProxyUserName(const Value: AnsiString);
begin
  tgputty_conf_set_str(GetPuttyConfIndex(cPuttyConf_proxy_username),
    PAnsiChar(Value), @Fcontext);
end;

procedure TTGPuttySFTP.SetStat(const AFileName: AnsiString;
  const Attrs: fxp_attrs);
begin
  FLastMessages := '';
  Fcontext.fxp_errtype := cDummyClearedErrorCode; // "clear" error field
...
procedure TTGPuttySFTP.SetUnixMode(const AFileName: AnsiString;
  const AMode: Integer);
var
  Attrs: fxp_attrs;
begin
...
procedure TTGPuttySFTP.UploadFile(const ALocalFilename, ARemoteFilename
  : AnsiString; const anAppend: Boolean);
var
  res: Integer;
begin
  FLastMessages := '';
  Fcontext.fxp_errtype := cDummyClearedErrorCode; // "clear" error field
  res := tgsftp_putfile(PAnsiChar(ALocalFilename), PAnsiChar(ARemoteFilename),
    anAppend, @Fcontext);
  if res <> 1 then
    raise TGPuttySFTPException.Create(MakePSFTPErrorMsg('tgsftp_putfile'));
end;

procedure TTGPuttySFTP.UploadStream(const ARemoteFilename: AnsiString;
  const AStream: TStream; const anAppend: Boolean);
var
  res: Integer;
begin
...
procedure TTGPuttySFTP.SetBooleanConfigValue(const OptionName: AnsiString;
  const OptionValue: Boolean);
begin
  tgputty_conf_set_bool(GetPuttyConfIndex(string(OptionName)), OptionValue,
    @Fcontext);
end;
...
procedure TTGPuttySFTP.xfer_upload_data(const xfer: TSFTPTransfer;
  const buffer: Pointer; const len: Integer; const anoffset: UInt64);
begin
  tgputty_xfer_upload_data(xfer, buffer, len, anoffset, @Fcontext);
end;

function TTGPuttySFTP.xfer_upload_init(const fh: TSFTPFileHandle;
  const offset: UInt64): TSFTPTransfer;
begin
  Result := tgputty_xfer_upload_init(fh, offset, @Fcontext);
end;

function TTGPuttySFTP.xfer_download_init(const fh: TSFTPFileHandle;
  const offset: UInt64): TSFTPTransfer;
begin
  Result := tgputty_xfer_download_init(fh, offset, @Fcontext);
end;

function TTGPuttySFTP.xfer_download_preparequeue
  (const xfer: TSFTPTransfer): Boolean;
begin
  Result := tgputty_xfer_download_preparequeue(xfer, @Fcontext);
end;

function TTGPuttySFTP.xfer_download_data(const xfer: TSFTPTransfer;
  var buffer: PByte; var len: Int32): Boolean;
begin
